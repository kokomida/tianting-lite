...
class LayeredMemoryManager:
    ...
    def remember(
        self, content: str, tags: List[str], context_path: str = ""
    ) -> Dict[str, Any]:
        """
    ...
    def recall(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if remaining_limit > 0:
            app_results = self._jsonl_dao.search_memories(
                query, "application", remaining_limit
            )
        ...
        remaining_limit = limit - len(results)
        if remaining_limit > 0:
            archive_results = self._jsonl_dao.search_memories(
                query, "archive", remaining_limit
            )
        ...
        if remaining_limit > 0:
            for memory_id, memory in self._session_memory.items():
                ...
                if (
                    ...
                    or query_lower in memory["context_path"].lower()
                ):
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        total_memories = (
            len(self._session_memory)
            + len(self._core_memory)
            + len(self._app_memory)
            + len(self._archive_memory)
        )
    ...
    def load_layer(self, layer: str, force_reload: bool = False) -> Dict[str, Any]:
        ...
        if layer == "session":
            ...
            # Unknown layer
            return {"layer": layer, "count": 0, "loaded": False, "memory_ids": []}
