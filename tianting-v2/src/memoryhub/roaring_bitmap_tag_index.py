...
This module provides a compressed bitmap-based index for fast tag intersection
operations.
...
class RoaringBitmapTagIndex:
    ...
    def __init__(self, enable_compression: bool = True):
        ...
        logger.info(
            f"RoaringBitmapTagIndex initialized (roaring: {self._roaring_available})"
        )
    ...
    def find_memories_by_tags(
        self, tags: Set[str], operation: str = "intersection"
    ) -> Set[int]:
        """
    ...
    def get_tag_stats(self) -> Dict[str, int]:
        ...
        if self._roaring_available:
            ...
            return {
                tag: len(memory_ids)
                for tag, memory_ids in self._tag_to_memory_ids.items()
            }
    ...
    def get_memory_efficiency(self) -> Dict[str, float]:
        ...
        if self._memory_count > 0:
            total_tag_associations = sum(
                len(tags) for tags in self._memory_to_tags.values()
            )
            stats["average_tags_per_memory"] = (
                total_tag_associations / self._memory_count
            )
...
class RoaringBitmapTagIndexFactory:
    ...
    def check_dependencies() -> Dict[str, bool]:
        ...
        """
        return {"pyroaring": ROARING_AVAILABLE, "fallback_available": True}
